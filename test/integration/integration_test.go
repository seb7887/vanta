...
func TestPluginConfigurationSystemIntegration(t *testing.T) {
	...
	t.Run("CompleteConfigurationWorkflow", func(t *testing.T) {
		...
		results, err := plugins.LoadPluginsFromConfig(pluginConfigs)
		if err != nil {
		...
		for _, pc := range pluginConfigs {
			if err := plugins.ValidatePluginConfig(pc.Name, pc.Config); err != nil {
		...
		for _, pc := range pluginConfigs {
			defaults := plugins.GetDefaultConfig(pc.Name)
			if len(defaults) == 0 {
		...
		if err := plugins.ValidateConfigForHotReload("auth", pluginConfigs[0].Config, authConfig); err != nil {
	...
	t.Run("ConfigurationErrorHandling", func(t *testing.T) {
		...
		err = plugins.ValidatePluginConfig("auth", emptyConfig)
		if err == nil {
			t.Error("Expected validation error for empty auth configuration")
		}

		// Test plugin creation with invalid config
		_, err = plugins.CreatePluginFromConfig("auth", invalidConfig)
		if err == nil {
